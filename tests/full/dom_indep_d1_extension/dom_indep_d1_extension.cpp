...
TEST_CASE("Test full verification (dom_indep_d1_extension)",
          "[dom_indep_d1_extension]") {
  const std::string config_file_name =
      "tests/full/dom_indep_d1_extension/dom_indep_d1_extension.json";
  ...
  SECTION("Robust Probing Model with no minimization") {
    ...
    Test test = TestDomIndepd1Extension(settings);
  ...
  SECTION("Robust Probing Model with trivial minimization") {
    ...
    if (settings.GetTestOrder() == 1) {
      // We expect no leakage in the first order.
      REQUIRE(test.Require(false, 4, 8, 12, 8));
    } else {
      // We expect leakage in the second order.
      REQUIRE(test.Require(true, 4, 8, 12, 12));
    }
  }
  ...
  SECTION("Robust but Relaxed Probing Model with trivial minimization") {
    ...
    if (settings.GetTestOrder() == 1) {
      // We expect no leakage in the first order.
      REQUIRE(test.Require(false, 30, 8, 45, 8));
    } else {
      // We expect leakage in the second order.
      REQUIRE(test.Require(true, 30, 8, 45, 12));
    }
  }
