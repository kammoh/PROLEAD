...
Test TestDomIndepd1NonFlatten(Settings& settings) {
  const std::string library_file_name = "library.json";
  const std::string library_name = "NANG45";
  const std::string design_file_name =
      "tests/full/dom_indep_d1_non_flatten/dom_indep_d1_non_flatten.v";
  const std::string topmodule_name = "dom_indep_d1_non_flatten";
...
TEST_CASE("Test full verification (dom_indep_d1_non_flatten)",
          "[dom_indep_d1_non_flatten]") {
  const std::string config_file_name =
      "tests/full/dom_indep_d1_non_flatten/dom_indep_d1_non_flatten.json";
  ...
  SECTION("Robust Probing Model with no minimization") {
    ...
    Test test = TestDomIndepd1NonFlatten(settings);
