...
argparser.add_argument(
    ...
    type=Path,
    default=None,
    help="Just plot data and exist. The argument value should be the path to npz file containing PROLEAD data to plot",
)
...
argparser.add_argument(
    "--prolead-root-dir", help="Path to PROLEAD source directory", type=Path, default=SCRIPT_DIR
)
    ...
    if args.netlist is None:
        ...
        if liberty_lib is None:
            if prolead_root_dir is None:
                print(
                    "Neither --yosys-verilog-lib/--yosys-lib nor --prolead-root-dir where specified"
                )
                exit(1)
    ...
    if args.ports_json:
        ...
        with open(args.ports_json, "r") as f:
            ...
            if isinstance(shares_maps, dict):
                ...
                shares_maps = []
            for k, v in (j.get("inputs") or p.get("inputs", p.get("input", {}))).items():
                print(f"** Input: {k} -> {v}")
                v["direction"] = "input"
                jports.append({"name": k, **v})
            for k, v in (j.get("outputs") or p.get("outputs", p.get("output", {}))).items():
                v["direction"] = "output"
                jports.append({"name": k, **v})
            exclude_signals_regex = j.get("exclude_signals", "")
        for p in jports:
            name = p["name"]
            width = p.get("width")
    ...
    for p_name, p in ports_map.items():
        for regex, d in shares_maps:
            ...
            if m:
                ...
                break
        share_id = p.get("share_id", p.get("share"))
        if not p.get("width"):
            name, end, start = Port.range_from_name(name)
